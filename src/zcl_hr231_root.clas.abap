CLASS zcl_hr231_root DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      _get_xtt IMPORTING iv_filter     TYPE string
               RETURNING VALUE(ro_xtt) TYPE REF TO zif_xtt,

      _make_email_json IMPORTING iv_filter      TYPE string
...
CLASS ZCL_HR231_ROOT IMPLEMENTATION.
...
*      ENDIF.
      ...
      " № 1.1
      IF lv_error IS INITIAL.
        lv_error = zcl_hr231_options=>get_instance( )->check_authorization(
           iv_emergrole_id = ls_item-eid ).
      ENDIF.
...
*    ENDIF.
    ...
    IF lv_error IS INITIAL.
      lv_error = zcl_hr231_options=>get_instance( )->check_authorization(
         iv_emergrole_id = ls_9018_io-emergrole_id ).
    ENDIF.
...
**********************************************************************
    ...
    " Send also a color to result
    DATA(lr_result) = NEW zc_hr231_emergency_role( CORRESPONDING #( ls_9018_io ) ).
    SELECT SINGLE color INTO lr_result->color
    FROM zdhr231_emergrol
    WHERE emergrole_id = ls_9018_io-emergrole_id.
...
**********************************************************************
    ...
    ASSERT iv_filter IS NOT INITIAL.

    CASE ls_key-pernr.
      WHEN '00000000'. " REPORT
        DATA(lv_content) = _get_xtt( iv_filter )->get_raw( ).
      ...
      WHEN '77777777'. " SEND_EMAIL
        lv_content   = _make_email_json( iv_filter ).
        lv_mime_type = 'application/json'.

        io_srv_runtime->set_header(
             VALUE #( name  = 'Content-Disposition'
                      value = |inline; filename="ok.json"| ) ).
  ...
  METHOD _check_has_default.
    if ir_9018_io->emergrole_id IS INITIAL.
      rv_error = |Emergency role ID is not passed|.
    endIF.
...
*    rv_error = |For Personnel Number { ir_9018_io->pernr ALPHA = OUT } there is no default value in ZDHR231_EMR_DEF table|.
  ...
  METHOD _check_is_exists.
    ...
    IF lines( lt_prev_tab )    = 1                     AND
       ...
       is_9018_io-emergrole_id = is_9018_io-eid        AND
  ...
  METHOD _delete_previous.
    rv_error = zcl_hr231_options=>get_instance( )->check_authorization(
...
***********************************
  ...
  METHOD _get_root.
    ...
    IF sy-subrc <> 0.
      lt_alv = VALUE #( ( ename    = 'No data found'
                          grp_text = 'Error'  ) ).
    ENDIF.
  ...
  METHOD _get_xtt.
    ...
    ro_xtt = NEW zcl_xtt_excel_xlsx( NEW zcl_xtt_file_smw0( 'ZR_HR231_EMERGDUTY.XLSX' ) ).

    ro_xtt->merge( lt_sheets
         )->merge( iv_block_name = 'P' is_block = ls_paris_info ).
  ENDMETHOD.


  METHOD _make_email_json.
    SELECT pernr, begda, endda, role_text, ename INTO TABLE @DATA(lt_alv)
    FROM zc_hr231_emergency_role
    WHERE (iv_filter) AND ord NE @space
    ...
    DATA(ls_root) = VALUE ts_root(
      t     = CORRESPONDING #( lt_alv )
      period = lcl_util=>get_from_filter( iv_filter = iv_filter
                                          iv_find   = |PHOTO_PATH <> '| )
*      begda  = lcl_util=>get_from_filter( iv_filter = iv_filter
*                                          iv_find   = |BEGDA <= '| )
*      endda  = lcl_util=>get_from_filter( iv_filter = iv_filter
*                                          iv_find   = |ENDDA >= '| )
    ).

    DATA(lo_opt) = zcl_hr231_options=>get_instance( ).
    ASSIGN lo_opt->t_email[ emergrole_id = 00 ] TO FIELD-SYMBOL(<ls_email>).

    DATA(lv_body) = lcl_util=>format( iv_template = <ls_email>-body
                                      is_root     = ls_root ).
    DATA(lv_subject) = lcl_util=>format( iv_template = CONV #( <ls_email>-subject )
                                         is_root     = ls_root ).

    DATA(lt_recipient) = VALUE zcl_xtt=>tt_recipients_bcs( ).
    TRY.
        APPEND VALUE #( recipient  = cl_cam_address_bcs=>create_internet_address( <ls_email>-recipient ) ) TO lt_recipient[].

        _get_xtt( iv_filter )->send( io_sender         = cl_cam_address_bcs=>create_internet_address( <ls_email>-sender )
                                     iv_body           = lv_body
                                     iv_subject        = CONV #( lv_subject )
                                     it_recipients_bcs = lt_recipient
                                     iv_commit         = abap_true ).
        DATA(lv_message) = |Message was sent to { <ls_email>-recipient }|.
        DATA(lv_error)   = |false|.
      CATCH cx_address_bcs INTO DATA(lo_error).
        lv_message = lo_error->get_text( ).
        lv_error   = |true|.
    ENDTRY.

    rv_json = zcl_eui_conv=>string_to_xstring( '{ "error": ' && lv_error && ', "message":"' &&
                            escape( val    = lv_message
